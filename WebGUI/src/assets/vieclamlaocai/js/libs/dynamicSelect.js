...
class DynamicSelect {
  ...
  _template() {
    ...
    let template = `
            <div class="dynamic-select ${this.selectElement?.className} ${this.name}"${this.selectElement.id ? ' id="' + this.selectElement.id + '"' : ''} style="${this.width ? 'width:' + this.width + ';' : ''}${this.height ? 'height:' + this.height + ';' : ''}">
